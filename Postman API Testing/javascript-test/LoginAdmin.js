...
try {
    ...
    pm.test("El token es de tipo Bearer", function () {
        pm.expect(jsonResponse).to.have.property("token_type"); // Verifica que el campo exista
    ...
    pm.test("El campo 'redirect_to' existe y tiene un valor válido", function () {
        ...
        pm.expect(jsonResponse.redirect_to).to.be.a("string").that.includes("http://127.0.0.1:8000/dashboard");
    });
